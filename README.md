...
[![Version](https://img.shields.io/badge/version-3.0.0-blue.svg)](https://github.com/nkliu1772/LandLocator)
...
- 🔗 **智能引導線標籤 (預設)** - 文字可拖曳移動，與地號中心保持連線，完美解決重疊問題
- 🧠 **智能碰撞偵測** - 自動偵測標籤重疊，8 個方向智能選擇最佳位置
- 🎨 **引導線自訂** - 線條粗細 4px（預設）、最深色 #333、透明度 20%
- 📸 **地圖截圖功能** - 一鍵匯出純地圖畫面，檔名包含縣市地段資訊
- 🎯 **側邊欄摺疊** - 查詢完可摺疊左側欄位，方便截圖
- 🎯 **可拖曳標記** - 支援手動微調標記位置
...
| 📷 截圖 | 一鍵匯出純地圖畫面，檔名格式：縣市_地段_月日_時分.png |
| 標記樣式 | 選擇標記樣式（文字標籤/標準 Marker/Marker+文字/小圓點/引導線標籤） |
| 標記設定 ⚙️ | 自訂標記文字、顏色、編號起始值、引導線設定 |
| 側邊欄摺疊 ‹› | 點擊左側欄位右邊的圓形按鈕可摺疊/展開查詢區 |
...
- **🔗 引導線設定**（僅在使用引導線標籤樣式時有效）
  - **線條粗細**：1-5px 可調整（預設 4px，更清晰）
  - **線條顏色**：深灰/灰色/淺灰/紅色/藍色（預設深灰 #333，更明顯）
  - **透明度**：0-70% 可調整，數值越高越透明（預設 20%，線條清晰可見）
...
- **dom-to-image-more 2.8.0** - 地圖截圖功能（對 Leaflet 支援更好）
...
### v3.0.0 - 智能碰撞偵測與進階功能 (Current)
- 🧠 **智能碰撞偵測演算法** - 8 個方向自動選擇不重疊位置
- 📸 **地圖截圖功能** - 一鍵匯出純地圖畫面，檔名含縣市地段
- 🎯 **側邊欄摺疊** - 方便截圖時隱藏查詢區
- 🎨 **引導線優化** - 預設 4px 粗線條、最深色、20% 透明度
- 🔗 **預設引導線標籤** - 引導線標籤成為預設樣式
- 🐛 **修正座標 bug** - 解決字串串接導致的座標錯誤
- 🔬 **實驗性功能** - 磁力排斥演算法（已實作但預設關閉）
